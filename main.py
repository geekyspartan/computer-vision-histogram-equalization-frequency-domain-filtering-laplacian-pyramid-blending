...
def laplacian_pyramid_blending(img_in1, img_in2):
   ...
   for i in xrange(5,0,-1):
      ...
      tmp = cv2.pyrUp(gpImg1[i])
      lpImg1.append(cv2.subtract(gpImg1[i-1],tmp))
            
      tmp = cv2.pyrUp(gpImg2[i])
      lpImg2.append(cv2.subtract(gpImg2[i-1],tmp))
   ...
   for i in xrange(1,6):
      ...
      img_out = cv2.add(img_out, laplacianList[i])
